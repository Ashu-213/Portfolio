...
const MiniTerminal = () => {
  ...
  const [history, setHistory] = useState<HistoryItem[]>([
    { type: 'output', text: 'AshutoshOS v1.0.0 (tty1)' },
  ...
  const handleCommand = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      addLine(`guest@ashutosh:~$ ${input}`, 'input');
      ...
      switch (cmd) {
        ...
        case 'whoami':
          addLine('Ashutosh | 3rd Year Int M.Tech CSE @ VIT Bhopal');
          addLine('Status: Grinding LeetCode | Software Engineer');
          break;

        case 'skills':
          addLine('► React, Node.js, Java, Python, Web-Socket');
          addLine('► AI/ML: OpenCV, LLMs, YOLOv8');
          break;

        case 'projects':
          addLine('1. RnadomChat (P2P Video Chat App)');
          addLine('2. InterviewPrep (Interview Prepration App)');
          addLine('3. RxVault (Digital Perscription Managment System)');
          addLine('4. Intelligaurd (AI Survillance System)');
          break;
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-black border-4 border-black rounded-2xl shadow-neo overflow-hidden flex flex-col font-mono text-sm sm:text-base">
      <div className="bg-gray-200 border-b-4 border-black px-4 py-2 flex items-center justify-between">
        ...
        <span className="font-bold text-black text-xs tracking-widest">root@ashutosh:~</span>
      ...
      <div className="p-4 h-64 overflow-y-auto bg-black text-custom-blue custom-scrollbar">
        ...
        <div className="flex items-center mt-2">
          <span className="text-custom-green mr-2">guest@ashutosh:~$</span>
          <input
            type="text"
            value={input}
